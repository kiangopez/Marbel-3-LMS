...
if(isset($_POST['submit'])) {
    ...
    $sql = "SELECT * FROM students_tbl WHERE USN = $receiver";
    $res = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($res);
    $receiver_name = $row['fname']." ".$row['lname'];

    $sql1 = "SELECT * FROM students_tbl WHERE USN = $usn";
    $res1 = mysqli_query($conn, $sql1);
    $row1 = mysqli_fetch_assoc($res1);
    $sender_name = $row1['fname']." ".$row1['lname'];
    ...
    if (!mysqli_stmt_prepare($stmt, $sql2)) {
        header("location:".SITEURL."student/chat-student.php?error=stmtfailed");
        exit();
    } 
    ...
    if (!mysqli_stmt_prepare($stmt2, $sql3)) {
        header("location:".SITEURL."student/chat-student.php?error=stmtfailed2");
        exit();
    } 
    ...
    header("location:".SITEURL."student/chat-student.php?id=$id");
