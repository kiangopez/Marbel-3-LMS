...
if(isset($_POST['submit'])) {
    ...
    if($checkPwd === false) {
        header("location:".SITEURL."admin/change-password.php?error=wrongcurrentpassword&id=$id");
    ...
    if (pwdMatch($new_password, $confirm_password) !== false ) {
        header("location:".SITEURL."admin/change-password.php?error=passwordsdontmatch&id=$id");
        $_SESSION['pwd-error'] = "<p class='error'>Passwords do not match</p>";
        exit();
    }

    if($np < 6) {
        header("location:".SITEURL."admin/change-password.php?error=passwordmin&id=$id");
        $_SESSION['pwd-error'] = "<p class='error'>Passwords should contain atleast 6 characters</p>";
        exit();
    } else if ($np > 16) {
        header("location:".SITEURL."admin/change-password.php?error=passwordmax&id=$id");
    ...
    if (!mysqli_stmt_prepare($stmt, $sql1)) {
        header("location:".SITEURL."admin/change-password.php?error=stmtfailed&id=$id");
        exit();
    } 
